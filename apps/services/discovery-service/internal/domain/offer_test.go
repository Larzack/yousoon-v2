...
func TestNewOffer(t *testing.T) {
	...
	validity, _ := NewValidity(time.Now(), time.Now().Add(30*24*time.Hour), "Europe/Paris")
...
func TestNewPercentageDiscount(t *testing.T) {
	discount := NewPercentageDiscount(25)
...
func TestNewFixedDiscount(t *testing.T) {
	discount := NewFixedDiscount(200) // 2.00€ discount
...
func TestValidity_IsActive(t *testing.T) {
	futureValidity, _ := NewValidity(
		time.Now().Add(24*time.Hour),
		time.Now().Add(48*time.Hour),
		"Europe/Paris",
	)

	if futureValidity.IsActive() {
		t.Error("Validity.IsActive() should return false for future dates")
	}

	currentValidity, _ := NewValidity(
		time.Now().Add(-24*time.Hour),
		time.Now().Add(24*time.Hour),
		"Europe/Paris",
	)

	if !currentValidity.IsActive() {
		t.Error("Validity.IsActive() should return true for current period")
	}
}
